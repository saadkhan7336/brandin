...
const InfluencerRequestCard = ({
  ...
  // In the influencer dashboard, we are interacting with the brand.
  const otherParty = request.brandUser || {};
  const roleProfile = request.brandProfile || {};

  const brandName = otherParty.fullname || "Premium Brand";
  const logo = otherParty.profilePic;

  // Campaign details
  const campaignName =
    request.campaignDetails?.name || request.title || "Collaboration";
  const campaignId = request.campaignDetails?._id || request.campaign;
    ...
    <div className="bg-white rounded-[24px] border border-gray-100 p-5 md:p-6 mb-4 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.02)] transition-all hover:shadow-[0_8px_20px_-6px_rgba(0,0,0,0.06)] hover:border-blue-100/50 group">
      <div className="flex flex-col md:flex-row md:items-center gap-6">
        ...
        <div className="flex items-center gap-4 min-w-0">
          <div className="relative shrink-0">
            <UserAvatar
              src={logo}
              name={brandName}
              className="w-16 h-16 rounded-[20px] border-2 border-white shadow-sm ring-1 ring-gray-100"
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-0.5 flex-wrap">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                <span
                  className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${statusStyles.bg} ${statusStyles.text} ${statusStyles.border}`}
                >
                  {statusStyles.label}
                </span>
            ...
            <div className="flex items-center text-[13px] text-gray-500 font-medium">
              ...
              <span className="truncate">
                {roleProfile.industry || "Lifestyle & Fashion"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <button
            onClick={() =>
              navigate(`/influencer/search/campaign/${campaignId}`)
            }
            className="flex items-center gap-1.5 text-blue-600 mb-1 group/link w-fit"
          >
            <span className="text-[15px] font-bold tracking-tight truncate group-hover/link:underline">
              {campaignName}
            </span>
            ...
            {status === "accepted" && collaborationId ? (
              ...
              String(sender) !== String(userId) && (
                <>
                  ...
                  <button
                    onClick={() => onCounterOffer && onCounterOffer()}
                    className="flex-1 md:flex-none flex items-center justify-center gap-1.5 px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white text-[13px] font-bold rounded-xl transition-all shadow-sm shadow-amber-200"
                  >
                    <RotateCcw className="w-3.5 h-3.5" />
                    Reconsider
                  </button>
