...
const CampaignCard = ({
  ...
  const startDateStr = campaign.campaignTimeline?.startDate
    ? new Date(campaign.campaignTimeline.startDate).toLocaleDateString(
        "en-US",
    ...
    <div className="bg-white rounded-[24px] border border-gray-100 overflow-hidden flex flex-col md:flex-row shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] w-full relative group transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_40px_-8px_rgba(0,0,0,0.12)] hover:border-blue-100 cursor-pointer">
      ...
      <div className="relative w-full md:w-[190px] bg-gradient-to-br from-blue-700 to-indigo-800 p-5 flex flex-col items-center justify-center shrink-0 transition-all duration-500 group-hover:from-blue-600 group-hover:to-indigo-700">
        ...
        <div className="relative z-10 transition-transform duration-500 group-hover:scale-110">
          {brandLogo ? (
            <img
              loading="lazy"
              decoding="async"
              src={getOptimizedImage(brandLogo, "avatar")}
              alt={brandName}
        ...
        <div className="mt-3 text-center z-10">
          <div className="text-white/70 text-[8px] font-black uppercase tracking-[0.2em] mb-1">
            CAMPAIGN BY
          </div>
          <div className="flex items-center justify-center gap-1.5">
            ...
            <VerifiedTick
              user={campaign.brandUser}
              roleProfile={campaign.brandProfile}
              size="xs"
            />
          </div>
        </div>
          ...
          <div className="flex flex-col gap-1 min-w-0">
            ...
            <p className="text-[12px] text-gray-500 leading-relaxed line-clamp-2">
              {campaign.description ||
                `Join this exciting ${campaign.industry || "brand"} campaign and showcase your talent.`}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-50 flex items-center gap-3">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              isCollaboration
                ? onViewCollaboration(collaborationId)
                : !isApplied && onApply(campaign);
            }}
...
const BrandCard = ({
  ...
  const name =
    brand.fullname ||
    brand.brandname ||
    (brand.user && brand.user.fullname) ||
    "Brand";
  const logo =
    brand.profilePic || (brand.user && brand.user.profilePic) || brand.logo;
  ...
  const effectiveCollabId =
    collaborationId || collaborationStatus?.collaborationId;
  ...
  if (brand.socialMedia) {
    const socialKeys =
      typeof brand.socialMedia.keys === "function"
      ...
      <div className="px-5 pb-5 flex-1 flex flex-col -mt-10 relative items-center text-center">
        ...
        <div className="relative mb-2 transition-transform duration-500 group-hover:scale-105">
          {logo ? (
            <img
              loading="lazy"
              decoding="async"
              src={getOptimizedImage(logo, "avatar")}
              alt={name}
        ...
        <div className="mb-3 px-3 w-full">
          <p className="text-[10px] text-gray-500 leading-normal line-clamp-2 overflow-hidden h-[30px]">
            {brand.description ||
              `Leading ${brand.industry || "brand"} looking for influencers to collaborate on exciting campaigns.`}
          </p>
          <button
            onClick={(e) => {
              e.stopPropagation();
        ...
        <div className="grid grid-cols-2 gap-2.5 w-full mb-2">
          ...
          <div className="p-2.5 bg-white rounded-[14px] border border-gray-100 flex items-center gap-2.5 shadow-sm transition-all duration-300 hover:bg-rose-50/30 hover:border-rose-100 hover:scale-[1.02] hover:shadow-md cursor-default group/stat3 col-span-1">
            ...
            <div className="flex flex-col text-left truncate">
              ...
              <span
                className="text-[10px] font-black text-gray-900 leading-none mt-0.5 truncate"
                title={brand.address || brand.location || "N/A"}
              >
        ...
        <div className="mt-auto flex gap-3 w-full">
          ...
          <button
            onClick={() =>
              isOngoing
                ? onViewCollaboration(effectiveCollabId)
                : isPreviouslyWorked
                  ? onViewCollaboration(effectiveCollabId)
                  : !isRequested && onSendRequest(brand)
            }
...
const SearchExplore = () => {
  ...
  const [selectedApplicationCampaign, setSelectedApplicationCampaign] =
    useState(null);
  ...
  const handleAIMatch = async () => {
    ...
    try {
      ...
      const matches =
        res?.matches || res?.data?.matches || res?.data || res || [];
  ...
  useEffect(() => {
    const fetchExistingRequests = async () => {
      try {
        const res = await collaborationService.getRequests({
          type: "sent",
          limit: 100,
        });
        if (res.success) {
        ...
        if (res.success) {
          ...
          const mapping = collabs.map((c) => {
            ...
            return {
              ...
              campaignName:
                c.campaign?.name || c.campaign?.title || "Collaboration",
            };
          });
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Discover brands and campaigns to collaborate with
              </p>
            </div>
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-6 py-6">
        ...
        {(loading || aiLoading) && (
          ...
          >
            {Array.from({ length: 8 }).map((_, i) => (
              <SkeletonCard
                key={i}
                type={activeTab === "campaigns" ? "campaign" : "brand"}
              />
            ))}
          </div>
        )}
        ...
        {!(loading || aiLoading) &&
          ...
          (isAIMode
            ...
            : (activeTab === "campaigns" ? campaigns : brands).length ===
              0) && (
            <div className="text-center py-20">
              ...
              <p className="text-sm text-gray-400">
                {isAIMode
                  ? "Try adjusting your profile or explore normally"
              ...
              {aiCampaigns.map((item) => {
                ...
                return (
                  <CampaignCard
                    ...
                    isApplied={
                      !isCollab &&
                      appliedCampaignIds.map(String).includes(String(c._id))
                    }
                    ...
                    onApply={(campaign) =>
                      setSelectedApplicationCampaign(campaign)
                    }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {aiBrands.map((item) => {
                ...
                const localCollab = activeCollaborations.find(
                  (ac) =>
                    ac.brandId === String(b._id) ||
                    ac.brandId === String(b.user?._id || b.user || ""),
                );
                const isCollab =
                  !!localCollab ||
                  (collabStatus && ["Ongoing"].includes(collabStatus.label));
                return (
                  <BrandCard
                    key={b._id}
                    brand={b}
                    isRequested={
                      !isCollab &&
                      requestedBrandIds.map(String).includes(String(b._id))
                    }
                    isViewed={viewedIds.includes(b._id)}
                    isCollaboration={isCollab}
                    collaborationId={
                      localCollab?.id || collabStatus?.collaborationId
                    }
                    activeCampaignName={
                      localCollab?.campaignName || collabStatus?.campaignName
                    }
        ...
        <ApplyCampaignModal
          ...
          onSuccess={() => {
            setAppliedCampaignIds((prev) => [
              ...prev,
              selectedApplicationCampaign._id,
            ]);
