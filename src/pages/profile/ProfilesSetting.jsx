      ...
      >
        {preview ? (
          <img
            loading="lazy"
            decoding="async"
            src={getOptimizedImage(preview, isCircle ? "avatar" : "cover")}
            alt={label}
...
const inputClass =
  "w-full bg-gray-50 border border-gray-100 rounded-xl px-5 py-3 text-sm font-bold text-gray-700 " +
...
export default function ProfileSettings() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setPortfolio(
        Array.isArray(roleProfile.portfolio) ? roleProfile.portfolio : [],
      );
      ...
      if (data.user?.verifiedPlatforms) {
        ...
        setSelectedPlatforms(
          Array.from(platforms).filter((p) => supported.includes(p)),
        );
      } else {
        ...
        setSelectedPlatforms(
          Object.keys(sm).filter((p) => supported.includes(p)),
        );
      }
    } catch (err) {
  ...
  const saveUserInfo = async () => {
    ...
    try {
      ...
      if (user?.role === "brand") {
        ...
        dispatch(
          setProfileData({
            roleProfile: brandRes.brand,
            completion: brandRes.completion,
          }),
        );
        if (brandRes.user) {
          dispatch(updateUserFields(brandRes.user));
          dispatch(updateProfileComplete(brandRes.completion?.isComplete));
          // Re-sync fullname from server response
          if (brandRes.user.fullname) setFullname(brandRes.user.fullname);
        }
        ...
        // ── Re-sync local form state from server response ──────────────────
        const updatedProfile = infRes.influencer || {};
        setUsername(updatedProfile.username || "");
        setAbout(updatedProfile.about || "");
        setCategory(updatedProfile.category || "");
        setPortfolio(
          Array.isArray(updatedProfile.portfolio)
        ...
        if (infRes.user) {
          ...
          // Re-sync fullname from server response
          if (infRes.user.fullname) setFullname(infRes.user.fullname);
        }
      }

      // Sync fullname from the initial updateUserInfo call too
      if (userData.user?.fullname) setFullname(userData.user.fullname);
  ...
  const handleConnect = async (platform) => {
    ...
    const backendUrl = `${process.env.REACT_APP_API_URL || "https://brandy-backend.onrender.com"}/api/v1/oauth`;
  ...
  const handlePasswordChange = async () => {
    if (!oldPassword || !newPassword || !confirmPassword)
      return showToast("Fields missing", "error");
    ...
    <div className="w-full max-w-[1800px] mx-auto space-y-8 pb-24 px-4 md:px-8">
      ...
      <div className="flex flex-col gap-1 pt-8 border-b border-gray-100 pb-8">
        ...
        <p className="text-gray-500 font-medium text-sm">
          Control your platform identity and security settings.
        </p>
      </div>
          ...
          <div className="flex-1 space-y-6 max-w-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Field label="Username (Display Name)" required>
                    <input
                      ...
                      className={inputClass}
                      placeholder="e.g. @cool_influencer"
                    />
                  </Field>
            ...
            <div className="bg-gray-50/50 rounded-xl px-5 py-4 flex items-center gap-4 border border-gray-100">
              ...
              {user?.isVerified ? (
                ...
                <span className="text-[9px] font-bold uppercase tracking-wider text-amber-600 bg-amber-50 px-2.5 py-1 rounded-md">
                  Unverified
                </span>
              )}
            </div>
              ...
              <div className="p-6 bg-gray-50/50 rounded-2xl border border-gray-100 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        if (!portfolioLink.title || !portfolioLink.url)
                          return showToast("Enter title and URL", "error");
                        setPortfolio((prev) => [
                          ...prev,
                          { ...portfolioLink, type: "link" },
                        ]);
                ...
                {newPortfolioFiles.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                      Pending Uploads ({newPortfolioFiles.length})
                    </p>
                    {newPortfolioFiles.map((f, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between p-2 bg-white rounded-lg border border-gray-100 text-xs font-bold text-gray-600"
                      >
            ...
            <div className="space-y-4">
              ...
              newPortfolioFiles.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {(Array.isArray(portfolio) ? portfolio : []).map(
                    (item, idx) => (
                      <div
                        key={idx}
                        className="group relative bg-white p-4 rounded-2xl border border-gray-100 hover:border-blue-100 transition-all hover:shadow-md"
                      >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden transition-all duration-300 mb-8 mt-8">
        <div className="px-8 py-6 flex items-center justify-between border-b border-gray-50 bg-gray-50/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-gray-400 mt-0.5">
                Connect your social platforms via OAuth to get your Verified
                badge and boost brand trust.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-8">
          ...
          {(() => {
            const verifiedCount = selectedPlatforms.filter(
              (p) => verifiedPlatforms[p],
            ).length;
                    ...
                    <div className="flex items-center gap-3 px-4 py-2 bg-white rounded-xl border border-amber-100 shadow-sm">
                      <div className="text-right">
                        ...
                        <p className="text-[11px] font-black text-amber-600 uppercase tracking-wider">
                          Unverified ({verifiedCount}/{totalCount})
                        </p>
                      </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 bg-blue-50/20 rounded-2xl border border-blue-50">
              <div>
                ...
                <p className="text-[10px] font-medium text-gray-500">
                  Select which platforms you want to verify and display on your
                  profile.
                </p>
              </div>
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <select
                    ...
                    if (val && !selectedPlatforms.includes(val)) {
                      ...
                      try {
                        if (user?.role === "brand") {
                          ...
                          await profileService.updateInfluencerProfile({
                  ...
                  className={cn(
                    inputClass,
                    "w-full sm:w-48 py-2 px-4 shadow-none",
                  )}
                >
            ...
            <div className="space-y-3">
              ...
              {selectedPlatforms.map((platform) => {
                ...
                return (
                  <div
                    key={platform}
                    className="group flex flex-col sm:flex-row items-center justify-between p-4 bg-white rounded-2xl border border-gray-100 hover:border-blue-100 transition-all hover:shadow-md"
                  >
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs font-medium text-gray-400 truncate max-w-[200px]">
                          {isVerified
                            ? platformData.username || "Account Connected"
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            const newSelected = selectedPlatforms.filter(
                              (p) => p !== platform,
                            );
                            setSelectedPlatforms(newSelected);
          ...
          <div className="bg-blue-50/30 rounded-3xl p-8 space-y-6 border border-blue-50/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div
                  key={idx}
                  className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center gap-3 shadow-sm hover:scale-[1.02] transition-all"
                >
                  <div className="w-6 h-6 rounded-md bg-blue-50 flex items-center justify-center">
                    <Check size={12} className="text-blue-500" />
                  </div>
                  <p className="text-[11px] font-bold text-gray-600 tracking-tight leading-tight">
                    {item.label}
                  </p>
                </div>
              ))}
            ...
            <div className="flex justify-center flex-col items-center gap-2 border-t border-blue-100/50 pt-6">
              <p className="text-[10px] font-medium text-gray-400 flex items-center gap-1.5">
                <Info size={12} className="text-blue-400" /> Your data is
                processed securely through official platform APIs.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-red-50/30 rounded-2xl border border-red-100 p-8 md:p-10 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
          ...
          <div className="bg-white p-6 rounded-2xl border border-red-100 flex flex-col justify-between items-start gap-5 shadow-sm">
            <div>
              <h4 className="font-bold text-red-900 flex items-center gap-2 tracking-tight">
                <ShieldClose size={18} /> Deactivate Account
              </h4>
              <p className="text-xs font-medium text-gray-500 mt-2 leading-relaxed">
                Temporarily hide your profile and active campaigns. You can
            ...
            {showManageAccount === "deactivate" ? (
              <div className="flex gap-2 w-full">
                <button
                  ...
                  className="flex-1 bg-red-600 text-white text-xs font-bold uppercase py-2.5 rounded-lg hover:bg-red-700 transition-colors"
                >
                  Confirm
                </button>
                <button
                  onClick={() => setShowManageAccount(null)}
                  className="flex-1 bg-gray-100 text-gray-600 text-xs font-bold uppercase py-2.5 rounded-lg hover:bg-gray-200 transition-colors"
                >
                  Cancel
                </button>
              </div>
              ...
              <button
                onClick={() => setShowManageAccount("deactivate")}
                className="w-full py-2.5 bg-red-50 text-red-600 text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-red-100 transition-colors"
              >
                Deactivate Now
              </button>
            )}
          ...
          <div className="bg-white p-6 rounded-2xl border border-red-100 flex flex-col justify-between items-start gap-5 shadow-sm">
            <div>
              ...
              <p className="text-xs font-medium text-gray-500 mt-2 leading-relaxed">
                Careful: This will wipe your account, history, and
                collaborations forever from our database.
              </p>
            </div>
            {showManageAccount === "delete" ? (
              <div className="flex gap-2 w-full">
                <button
                  ...
                  className="flex-1 bg-red-900 text-white text-xs font-bold uppercase py-2.5 rounded-lg hover:bg-black transition-colors"
                >
                  Confirm Delete
                </button>
                <button
              ...
              <button
                onClick={() => setShowManageAccount("delete")}
                className="w-full py-2.5 bg-red-50 text-red-600 text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-red-100 transition-colors"
              >
                Delete Account
              </button>
        ...
        <div className="flex justify-center pt-2">
          <button
            ...
            className="flex items-center gap-2 text-red-500 hover:text-red-700 font-bold text-xs bg-white px-8 py-3 rounded-full border border-red-100 shadow-sm transition-all hover:shadow-md active:scale-95"
          >
